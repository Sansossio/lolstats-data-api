...
export class OriginMiddleware implements NestMiddleware {
  private isLocalhost (host: string) {
    let response = false
    for (const localhost of OriginAcceptedEnum.LOCALHOST) {
  ...
  private isAcceptanceHost (origin: string | undefined) {
    ...
    const index = origin.indexOf(OriginAcceptedEnum.ACCEPTANCE_HOST)
  ...
  use (req: Request, res: Response, next: Function) {
    ...
    if (!isValid) {
      throw new ForbiddenException(OriginAcceptedEnum.MESSAGE)
    }
    next()
  }
}
